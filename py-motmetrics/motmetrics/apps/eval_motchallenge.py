...
def main():
    ...
    gtfiles = ['/home/szh/桌面/py-motmetrics/yourdata/video1/gt/gt.txt',
               ]
    tsfiles = ['/home/szh/桌面/py-motmetrics/yourdata/video1.txt',
               ]
    ...
    ts = OrderedDict(
        [(os.path.splitext(Path(f).parts[-1])[0], mm.io.loadtxt(f, fmt=args.fmt, min_confidence=-1)) for f in tsfiles])
